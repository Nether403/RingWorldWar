...
- [ ] **Task: Implement `core/ringMath.ts`**
...
- [ ] **Task: Implement deterministic math + RNG**
  - Acceptance: seeded xorshift128+ RNG; `detMath` sin/cos/atan2 with documented precision; ESLint rule bans `Math.random` and raw `Math.sin/cos` inside `src/sim/`.
  - Verify: `npm run test tests/core/` — same seed produces identical sequences; lint fails on a deliberately-added `Math.random()` in a sim file.
  - Files: `src/core/rng.ts`, `src/core/detMath.ts`, `.eslintrc.cjs`, `tests/core/rng.test.ts`

- [ ] **Task: Spike A — floating-origin ring flythrough**
...
- [ ] **Task: Spike B — rotating-frame ballistics**
...
- [ ] **Task: Spike C — far-ring shell**
  - Acceptance: the entire 3,770 km ring is visible from any point on the surface, rendered via the baked shell, costing < 3 ms and < 20 draw calls.
...
- [ ] **GATE 0 review** — all spikes demonstrated; go/no-go on ring dimensions before Phase 1 begins.

---
## Phase 1 — Playable Core Loop

- [ ] **Task: ECS world + fixed-timestep loop**
...
- [ ] **Task: Move sim into a worker**
  - Acceptance: sim runs off the main thread over a SAB double-buffer with a structured-clone fallback; render reads a consistent snapshot; determinism test still passes.
  - Verify: `npm run test` + F3 overlay showing main-thread sim cost near zero.
  - Files: `src/sim/worker.ts`, `src/sim/snapshot.ts`, `src/main.ts`

- [ ] **Task: Terrain — heightfield, clipmap LOD, navgrid**
...
- [ ] **Task: Pathfinding — wrapped flow fields**
...
- [ ] **Task: Units, targeting, combat, fog of war**
...
- [ ] **Task: Victory conditions + match flow**
...
- [ ] **Task: Scripted opponent (pre-AI placeholder)**
